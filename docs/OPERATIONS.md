...
Trusting `127.0.0.1` also trusts every other program on this computer, as [Behind a reverse proxy](#behind-a-reverse-proxy) describes. While sharing is on, such a program can send forwarded headers to OwnGit: it can choose the client address that OwnGit uses for password lockouts, and make its request look like HTTPS. Tailscale Serve connects from `127.0.0.1` too, so OwnGit cannot tell the two apart. If `127.0.0.1` was already trusted, turning sharing on changes nothing here.

...
var importCatalog = map[MessageCode]message{
	...
	MsgImportIntro:              {en: "OwnGit keeps its own copy. It never writes to the source.", ko: "OwnGit는 자기 복사본을 둡니다. 원본에는 쓰지 않습니다."},
	...
	MsgImportGitOnlyHelp:        {en: "Git LFS pointers stay in the copy and the content is marked incomplete. OwnGit does not download LFS objects.", ko: "Git LFS 포인터는 복사본에 남고 내용은 불완전으로 표시됩니다. OwnGit는 LFS 객체를 받지 않습니다."},
	...
	MsgImportResolveHelp:        {en: "OwnGit could not confirm how an earlier publication ended, so refreshes are paused. Check the repository. If its current branches, tags, and HEAD are acceptable, accept them as they are. OwnGit does not change any ref for this. The next refresh plans from the repository as it is now, and it leaves refs that differ from both the source and the last confirmed import alone.", ko: "이전 게시가 어떻게 끝났는지 OwnGit가 확인하지 못해 새로고침을 멈췄습니다. 저장소를 확인하세요. 현재 브랜치, 태그, HEAD를 그대로 써도 된다면 현재 상태로 인정하세요. 이 작업은 ref를 바꾸지 않습니다. 다음 새로고침은 지금 저장소 상태를 기준으로 계획하며, 원본과도 마지막으로 확인된 가져오기와도 다른 ref는 그대로 둡니다."},
	...
	MsgImportSchedulerFailed:    {en: "The import scheduler did not start. Scheduled refreshes do not run until OwnGit restarts. Ordinary Git service is available.", ko: "가져오기 예약 실행기가 시작되지 않았습니다. OwnGit를 다시 시작할 때까지 예약 새로고침은 실행되지 않습니다. 일반 Git 서비스는 사용할 수 있습니다."},
	...
	MsgImportErrorProtocol:      {en: "The source sent a response that OwnGit could not accept.", ko: "원본이 OwnGit가 받아들일 수 없는 응답을 보냈습니다."},
	...
	MsgImportErrorInterrupted:   {en: "The run was interrupted when OwnGit stopped.", ko: "OwnGit가 멈추면서 실행이 중단되었습니다."},
	MsgImportErrorState:         {en: "OwnGit could not read or record import state.", ko: "OwnGit가 가져오기 상태를 읽거나 기록하지 못했습니다."},
	...
	MsgImportRefDiverged:        {en: "Diverged", ko: "달라짐"},
	MsgImportRefAbsent:          {en: "Absent locally", ko: "로컬에 없음"},
	MsgImportRefEarlier:         {en: "Earlier source", ko: "이전 원본"},
	MsgImportRefUnknown:         {en: "Local unknown", ko: "로컬을 확인하지 못함"},
}

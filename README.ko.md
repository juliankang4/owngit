...
<p align="center">
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/version-1.1.0-0A62C9?style=flat&colorA=222222" alt="버전 1.1.0"></a>
...
- Tailscale Serve로 tailnet 안에서 OwnGit을 HTTPS로 공유합니다. 설정 화면이나 `owngit tailscale on`으로 켭니다. Caddy, nginx, Traefik 같은 리버스 프록시 뒤에서도 OwnGit을 운영할 수 있습니다. `owngit network set`으로 저장한 네트워크 설정은 시작할 때마다 적용되므로, `brew services`로 띄운 백그라운드 서비스에도 다른 기기에서 접속할 수 있습니다. [tailnet에서 HTTPS로 공유하기](docs/OPERATIONS.ko.md#tailnet에서-https로-공유하기)와 [리버스 프록시 뒤에서 운영하기](docs/OPERATIONS.ko.md#리버스-프록시-뒤에서-운영하기)를 참고하세요.
- 저장소, 브랜치, 태그, 파일, 커밋, diff, 작성일 기준 활동, 풀 리퀘스트, 특정 리비전에 묶인 체크 결과를 브라우저에서 보여 줍니다.
- 브랜치, 태그, 커밋을 브라우저나 `curl`로 ZIP 또는 tar.gz 압축 파일로 내려받을 수 있습니다.
- 저장소 개요에서 기본 브랜치의 파일 크기를 기준으로 저장소에 쓰인 언어를 보여 줍니다. 데이터와 문서 파일(JSON, YAML, Markdown, 일반 텍스트)과 vendor, 자동 생성, 문서 경로는 세지 않으며, `.gitattributes`의 Linguist 속성은 Git 2.40 이상에서 반영합니다.
...
- `owngit repo`로 명령줄에서 저장소 목록을 보고, 저장소를 살펴보고, 새로 만들 수 있습니다. MCP를 지원하는 코딩 도구에 풀 리퀘스트, 저장소, 체크 명령을 `owngit mcp`로 제공합니다. 표준 입력과 표준 출력으로 동작하는 로컬 서버입니다. [코딩 도구 연동](docs/CODING_TOOLS.ko.md#mcp-서버)을 참고하세요.
...
소스에서 빌드했다면 `./bin/owngit serve`, 압축 파일을 풀었다면 `./owngit serve`로 실행합니다. 터미널에서 처음 실행하면 그 터미널에서 설치를 진행합니다. English나 한국어를 고른 뒤 "이 터미널에서 계속"과 "웹 대시보드 열기" 중 하나를 고르세요. 터미널에서 입력하는 비밀번호는 화면에 나타나지 않습니다. 웹 대시보드를 고르면 브라우저에 짧은 코드가 나오고, 터미널에서 그 브라우저를 승인해야 설치를 이어 갈 수 있습니다. 기본 주소는 `http://127.0.0.1:7654`입니다. 자세한 내용은 [처음 설정하기](docs/OPERATIONS.ko.md#처음-설정하기)를 보세요.

`brew services`처럼 터미널 없이 시작하면 OwnGit이 상태 디렉터리 안에 설치한 계정만 읽을 수 있는 설정 파일을 만들고 브라우저에서 엽니다. `--no-open`을 붙였거나 브라우저를 열 수 없으면 서버 로그에 파일 경로가 나옵니다. 설정용 비밀값은 화면에 출력되거나 브라우저 인수로 전달되지 않습니다.
...
OwnGit은 약 30 MB짜리 프로그램 하나(내려받는 크기는 18 MB)이고, 컴퓨터에 이미 있는 Git을 함께 씁니다. 아래 표는 OwnGit 1.0.3을 설정한 뒤 아무도 쓰지 않는 상태에서 메모리가 안정된 다음 잰 값입니다.

| | Linux x64 | macOS (Apple silicon) |
| --- | --- | --- |
| 메모리, 저장소 없음 | 약 45 MB | 약 55 MB |
| 메모리, 작은 저장소 100개 | 약 50 MB | 약 50에서 70 MB |
| CPU | 코어 하나의 0.1% 미만 | 코어 하나의 0.1% 미만 |

비밀번호를 확인할 때마다 Argon2id 계산 때문에 메모리를 잠깐 70 MB쯤 더 씁니다. 비밀번호를 정하거나 로그인할 때, 관리자 비밀번호로 설정 변경을 확인할 때, 그리고 공용 비밀번호를 정해 두었다면 Git과 API 요청에서 비밀번호를 확인합니다. 공용 비밀번호를 한 번 확인하면 같은 비밀번호는 5분 동안 다시 계산하지 않고 받아들이므로, 클론이나 푸시 한 번에 여러 요청이 오가도 확인은 한 번이면 됩니다. 한 번에 최대 네 개까지 동시에 확인하고, 쓴 메모리는 몇 분 뒤 운영체제에 돌려줍니다. 클론이나 푸시를 할 때는 Git도 실행되며, Git이 쓰는 메모리는 저장소에 따라 다릅니다. 메모리 값은 `/proc`과 `ps`가 보고하는 상주 메모리(RSS)입니다. macOS의 활동 모니터에는 더 큰 값이 보일 수 있습니다.

...
OwnGit은 약 30 MB짜리 프로그램 하나(내려받는 크기는 18 MB)이고, 컴퓨터에 이미 있는 Git을 함께 씁니다. OwnGit 1.0.3에서 설정을 마친 뒤 아무도 쓰지 않을 때 잰 값입니다.
...
비밀번호를 정하거나 로그인할 때는 비밀번호를 Argon2id로 처리하느라 잠깐 64 MB쯤 더 쓰고, 몇 분 안에 운영체제에 돌려줍니다. 클론이나 푸시를 할 때는 Git도 실행되며, Git이 쓰는 메모리는 저장소에 따라 다릅니다. 메모리 값은 `/proc`과 `ps`가 보고하는 상주 메모리(RSS)입니다. macOS의 활동 모니터에는 더 큰 값이 보일 수 있습니다.

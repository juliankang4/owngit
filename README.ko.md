...
처음 실행하면 OwnGit이 상태 디렉터리 안에 설치한 계정만 읽을 수 있는 설정 파일을 만들고 브라우저에서 엽니다. 그 화면의 안내를 따르세요. 설정용 비밀값은 화면에 출력되거나 브라우저 인수로 전달되지 않습니다. `--no-open`을 붙였거나 브라우저를 열 수 없으면 서버 로그에 파일 경로가 나옵니다. 기본 주소는 `http://127.0.0.1:7654`입니다.

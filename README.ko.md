...
- MCP를 지원하는 코딩 도구에 풀 리퀘스트, 저장소, 체크 명령을 `owngit mcp`로 제공합니다. 표준 입력과 표준 출력으로 동작하는 로컬 서버입니다. [코딩 도구 연동](docs/CODING_TOOLS.ko.md#mcp-서버)을 참고하세요.
...
- OwnGit은 암호화되지 않은 일반 HTTP로 동작하며 TLS를 내장하지 않습니다. TLS는 OwnGit 앞에 둔 리버스 프록시나 Tailscale이 맡습니다. OwnGit은 사용자가 지정한 프록시가 보낸 전달 헤더만 믿습니다([리버스 프록시 뒤에서 운영하기](docs/OPERATIONS.ko.md#리버스-프록시-뒤에서-운영하기) 참고). 다른 기기에서 접속할 때는 Tailscale이나 직접 운영하는 VPN을 쓰는 편이 좋습니다. 공개 인터넷에서 운영하는 용도는 지원 범위가 아닙니다.

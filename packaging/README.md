...
- Publish the four platform packages first and `owngit` last, so no install sees version pins that do not resolve yet. The `npm publish` workflow does this with npm trusted publishing; see [Releases in CONTRIBUTING.md](../CONTRIBUTING.md#releases). A published README cannot be changed for that version.

...
**Upgrading:** the state database stays at schema 15 and offline backups at version 10, as in 1.0.3, and OwnGit 1.1.0 upgrades the database of any earlier 1.0.x release directly. You can go back to 1.0.3 without restoring a backup. It ignores the network settings that 1.1.0 saved, such as the base URL and trusted proxies, so a listen address saved with `owngit network set` no longer applies under 1.0.3. Upgrading from 1.0.3 and going back were tested on Linux, Windows, macOS and a Raspberry Pi. Before you upgrade, stop OwnGit and make a backup with `owngit backup`.

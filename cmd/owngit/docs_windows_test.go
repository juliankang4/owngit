...
func TestWindowsDocumentedPasswordFileCommands(t *testing.T) {
	...
	for _, test := range []struct{ document, origin string }{
		...
		if len(blocks) != 1 {
			t.Fatalf("%s has %d PowerShell blocks, want 1", test.document, len(blocks))
		}
		home := t.TempDir()
		...
		if err != nil {
			t.Fatalf("%s: the documented commands failed: %v\n%s", test.document, err, output)
		}
		...
		if err != nil {
			...
			t.Fatalf("%s: the documented file was refused: %v\n%s", test.document, err, listing)
		}
		if file.secret != password || file.origin != test.origin {
			t.Fatalf("%s: read secret %q origin %q", test.document, file.secret, file.origin)
		}
	}
}
